...
def _gnomonic_to_cam_numpy128(x_gnom, y_gnom, dd, xcen, ycen, beta, gamma, pixelsize):
    ...
    x3 = 2*x2 - float128('1.41421356237309504880168872421')
    x4 = x3*y_gnom
...
def _cam_to_thetachi_numpy128(x_cam, y_cam, dd, xcen, ycen, beta, gamma, pixelsize):
    ...
    del x9, x6, x2, x1
    _0 = float128('28.6478897565411614323238609359')*arccos(x10/sqrt(x10**2 + x12))
    del x10
    _1 = float128('-57.2957795130823228646477218717')*arcsin(pixelsize*x11/sqrt(x12))
    return [_0, _1]
...
def _gnomonic_to_thetachi_numpy128(x_gnom, y_gnom):
    ...
    x0 = y_gnom**2
    x1 = x_gnom - 1
    _1 = float128('-57.2957795130823228646477218717')*arcsin(x1*y_gnom/(sqrt(x0 + 0.5*(x_gnom + 1)**2)*abs(x1)))
    del x1
    _0 = float128('28.6478897565411614323238609359')*arccos((x0 + 2*x_gnom)/(x0 + x_gnom**2 + 1))
    return [_0, _1]
...
def _dist_cosine_numpy128(theta_1, chi_1, theta_2, chi_2):
    ...
    x4 = cos(x3)
    x5 = -x4
    _0 = float128('57.2957795130823228646477218717')*arccos(0.5*(x1*x4 + x2 + x5 + sin(x0)*sin(x3)*cos(float128('0.0174532925199432954743716805979')*chi_1 - float128('0.0174532925199432954743716805979')*chi_2))/(sqrt(x2)*sqrt(x5 + 1)))
    return _0
...
def _hough_numpy128(x_a, y_a, x_b, y_b):
    ...
    del x6, x5, x4
    _1 = float128('1.41421356237309504880168872421')*sqrt(0.5*x10 + 0.5*x11 + 0.5*x12 - x7 - x8 + 0.5*x9)*abs((x_a*y_b - x_b*y_a)/(x10 + x11 + x12 - 2*x7 - 2*x8 + x9))
    return [_0, _1]
