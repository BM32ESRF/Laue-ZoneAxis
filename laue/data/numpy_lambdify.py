...
def _cam_to_thetachi_numpy(x_cam, y_cam, dd, xcen, ycen, beta, gamma, pixelsize):
    ...
    del x9, x6, x2, x1
    _0 = 28.6478897565412*arccos(x10*(x10**2 + x12)**(-0.5))
    del x10
    _1 = -57.2957795130823*arcsin(pixelsize*x11*x12**(-0.5))
    return [_0, _1]
...
def _gnomonic_to_thetachi_numpy(x_gnom, y_gnom):
    ...
    x0 = y_gnom**2
    x1 = x_gnom - 1.0
    _1 = -57.2957795130823*arcsin(1.0*x1*y_gnom*(x0 + 0.5*(x_gnom + 1.0)**2)**(-0.5)/abs(x1))
    del x1
    _0 = 28.6478897565412*arccos((x0 + 2.0*x_gnom)/(x0 + x_gnom**2 + 1.0))
    return [_0, _1]
...
def _dist_cosine_numpy(theta_1, chi_1, theta_2, chi_2):
    ...
    x4 = cos(x3)
    x5 = -x4
    _0 = 57.2957795130823*arccos(0.5*x2**(-0.5)*(x5 + 1.0)**(-0.5)*(x1*x4 + x2 + x5 + sin(x0)*sin(x3)*cos(0.0174532925199433*chi_1 - 0.0174532925199433*chi_2)))
    return _0
...
def _hough_numpy(x_a, y_a, x_b, y_b):
    ...
    del x6, x5, x4
    _1 = 1.4142135623731*sqrt(0.5*x10 + 0.5*x11 + 0.5*x12 - x7 - x8 + 0.5*x9)*abs((x_a*y_b - x_b*y_a)/(x10 + x11 + x12 - 2.0*x7 - 2.0*x8 + x9))
    return [_0, _1]
